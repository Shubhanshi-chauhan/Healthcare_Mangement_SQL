...
-- 3. Count the total number of patients in the database.
	Select count(*) AS Patients_name from Patients;
    
-- 4. Find the oldest patient in the database.
	Select patient_name, date_of_birth from Patients ORDER by date_of_birth LIMIT 1;
    
-- 5. Display the address and phone number of patient with ID 7.
	SeLect address, phone_number from Patients where patient_ID=7;
...
-- 11. Retrieve the names of patients along with their admission and discharge dates.
	Select patient_name, discharge_date,admission_date from Patients p JOIN
...
-- 13. List the patients who were diagnosed with hypertension or diabetes.
	Select p.patient_name,m.diagnosis from Patients p join MedicalRecords m on 
    p.patient_id= m.patient_id where diagnosis IN ('hypertension','diabetes');
...
-- 15. Display the doctors who treated patients admitted in January 2023.
	Select distinct d.* from Doctors d join MedicalRecords m on
    d.doctor_id =m.doctor_id where year(admission_date) = '2023' AND month(admission_date) = 1;

-- 16. Calculate the total number of patients treated by each doctor.
	Select doctor_id, count(*) AS total_patient_treated
    from MedicalRecords
    GROUP BY doctor_id;
...
select diagnosis , count(*) AS diagnosis_count from MedicalRecords
Group by diagnosis
